...
export default function EvalsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--color-text)]">Evaluations</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-xl p-5">
          <p className="text-xs text-[var(--color-text-subtle)] mb-1">Total Tests</p>
          <p className="text-3xl font-bold text-[var(--color-text)]">{stats.total}</p>
        </div>
        <div className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-xl p-5">
          <p className="text-xs text-[var(--color-text-subtle)] mb-1">Passed</p>
          <p className="text-3xl font-bold text-[var(--color-approved)]">{stats.passed}</p>
        </div>
        <div className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-xl p-5">
          <p className="text-xs text-[var(--color-text-subtle)] mb-1">Failed</p>
          <p className="text-3xl font-bold text-[var(--color-blocked)]">{stats.failed}</p>
        </div>
        <div className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-xl p-5">
          <p className="text-xs text-[var(--color-text-subtle)] mb-1">Pass Rate</p>
            ...
            return (
              <div key={cat.category}>
                <button
                  ...
                  className={`w-full px-6 py-4 text-left hover:bg-[var(--color-bg-sunken)] transition-colors ${
                    isSelected ? "bg-[var(--color-action-muted)]" : ""
                  }`}
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        ...
                        <div className="w-24 h-1.5 bg-[var(--color-border)] rounded-full mt-1">
                          <div
                ...
                {/* Expanded Test List */}
                {isSelected && (
                  <div className="px-6 pb-4 bg-[var(--color-bg-sunken)]">
                    <div className="space-y-2 pt-2">
                      {selectedCases.map((testCase) => (
                        <div
                          key={testCase.id}
                          className="bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-lg px-4 py-3"
                        >
